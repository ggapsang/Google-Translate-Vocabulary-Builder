...
/**
 * 테스트 세션 완료 후 vocabulary_stats를 업데이트합니다.
 * @param {Array} answers - [{ correct: boolean, ... }, ...]
 * @returns {Promise<boolean>}
 */
async function updateTestStats(answers) {
  try {
    ...
    // total.tests 증가
    stats.total.tests = (stats.total.tests || 0) + 1;

    // testStats 업데이트
    const oldTotal = stats.testStats.totalTests || 0;
    const oldAvg = stats.testStats.averageScore || 0;
    const newAvg = oldTotal === 0
      ? scorePercent
      : Math.round(((oldAvg * oldTotal) + scorePercent) / (oldTotal + 1));

    stats.testStats.totalTests = oldTotal + 1;
    stats.testStats.averageScore = newAvg;
    stats.testStats.bestScore = Math.max(stats.testStats.bestScore || 0, scorePercent);
    stats.testStats.worstScore = oldTotal === 0
      ? scorePercent
      : Math.min(stats.testStats.worstScore ?? scorePercent, scorePercent);

    // dailyActivity upsert
    const today = new Date().toISOString().split('T')[0];
    const dayEntry = stats.dailyActivity.find(d => d.date === today);
    if (dayEntry) {
      dayEntry.testsCompleted = (dayEntry.testsCompleted || 0) + 1;
    } else {
      stats.dailyActivity.push({ date: today, wordsAdded: 0, wordsReviewed: 0, testsCompleted: 1 });
    }

    await localSet({ [STORAGE_KEYS.STATS]: stats });
    console.log('[VocabBuilder] Test stats updated:', scorePercent + '%');
    return true;
  } catch (error) {
    console.error('[VocabBuilder] Error updating test stats:', error);
    return false;
  }
}
...
async function _updateDailyWords() {
  const stats = await getStats();
  const today = new Date().toISOString().split('T')[0];
  const dayEntry = stats.dailyActivity.find(d => d.date === today);
  if (dayEntry) {
